...
def match_region(ch, pattern):
    ...
    if pattern[1] == COMPLEMENT:
        booly = False; booln = True; i = 2
    else:
...
def match_simple_token(sarg, i, token):
    ...
    if c0 == ESCAPE:
        c1 = token[1]
        if c1 == 's' and ch in (' ', '\t', '\r', '\n', '\f', '\v'):
            MATCHLEN = 1; return True
        elif c1 == 'S' and ch not in (' ', '\t', '\r', '\n', '\f', '\v'):
            MATCHLEN = 1; return True
        elif '0' <= c1 <= '9':
            captured = xGroups[int(c1)];
            lg = len(captured)
            if sarg[i:i + lg] == captured: MATCHLEN = lg; return True
        elif ch == c1:
            MATCHLEN = 1; return True
    elif c0 == REGIONA and match_region(ch, token):
        MATCHLEN = 1; return True
    elif c0 == ANYCHAR or c0 == ch:
        MATCHLEN = 1; return True
    return False


def strip_groupattris(s):
    if any([s.startswith(EXTENSION + c) for c in (':', '=', '!')]):
        s = s[3:]
    else:
        s = s[1:]
...
def parse_pattern(pattern, nested):
    ...
    while i < pL:
        ...
        if c == REGIONA:
            ...
            tokens.append(c); i += 1
        if i < pL:
            if pattern[i] == PERHAPS:
                tokens[-1] = (tokens[-1], PERHAPS); i += 1
            elif pattern[i] == STAR:
                tokens[-1] = (tokens[-1], STAR); i += 1
            elif pattern[i] == JUST_ONE_and_STAR:
                tokens.append((tokens[-1], STAR));
                tokens[-2] = (tokens[-2], None);
                i += 1
            else:
                tokens[-1] = (tokens[-1], None)
        else:
            tokens[-1] = (tokens[-1], None)
    ...
    for tix in range(tkL + 1):
        ...
        if ns and ctk[0] == GROUPA:
            if any([try_index(sarg, t, True) for t in TokenListCache[ctk]]):
                ...
                if ctk.startswith(EXTENSION + '='):
                    continue
                elif ctk.startswith(EXTENSION + '!'):
                    return False
                compix = MATCHEND
                if qua == STAR:
...
def xfinditer(sarg, pattern, nested=False, start=0):
    ...
    while start < L:
        if n: start = n
        MATCHEND = start
        if try_index(sarg, tokens, nested):
            n = MATCHEND; yield (start, MATCHEND)
        else:
            n = 0; start += 1
    raise StopIteration()
...
def xsplit(sarg, pattern, nested=False):
    resu = [];
    xpair = xsearch(sarg, pattern, nested=nested);
    residue = 0
    while xpair:
...
def xreplace(sarg, pattern, subst, nested=False):
    to_replace = [];
    s = sarg;
    xpair = xsearch(sarg, pattern, nested)
    while xpair:
        to_replace.append(xpair);
        xpair = xsearch(sarg, pattern, nested, xpair[1])
    if nested:
