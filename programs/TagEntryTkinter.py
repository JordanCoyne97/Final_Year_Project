...
class Tag(Frame):
    ...
    def __init__(self, master, text):
        ...
        Label(self, text=text, background=self.BACKGROUND, foreground=self.FOREGROUND_LABEL).pack(side=LEFT, pady=(
        self.PADDING[1], self.PADDING[3]), padx=(self.PADDING[0], 0))

        self._close_IMG = PhotoImage(
            data="")
        Button(self, highlightthickness=0, borderwidth=0, background=self.BACKGROUND, activebackground=self.BACKGROUND,
               image=self._close_IMG, command=self.destroy).pack(side=LEFT, pady=(self.PADDING[1], self.PADDING[3]),
                                                                 padx=(self.LABEL_AND_BUTTON_SPACING, self.PADDING[2]))
...
class Tag_Entry(Frame):
    ...
    def add_tag(self, text):
        ...
        self.textarea.window_create("1.%s" % len(self.textarea.window_names()), window=tag,
                                    padx=self.SPACING_BETWEEN_TAGS)
