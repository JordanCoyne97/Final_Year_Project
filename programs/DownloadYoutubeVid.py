...
def main():
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 4:
        ...
        if sys.argv[1] == '-q':
            sys.argv[1] = '-q'; arg_queryf = sys.argv[1]
        else:
            return usage()
        if sys.argv[-1] == '-':
            url = list(sys.stdin.readlines())
...
def crawl_youtube(sTUBE):
    ...
    if vid_title:
        ...
        print('[-] ERROR: Can\'t find video title. Title set to default.'); final_title = 'DownloadYTube'
    ...
    if fmt:
        ...
        print('Can\'t find video formats. '); exit(1)
...
def download():
    ...
    try:
        ...
        urllib.request.urlretrieve(FINAL_LINK[final_f_format], (final_title.replace(' ', '') + \
                                                                '.' + VIDEO_TYPE[final_f_format]),
                                   reporthook=download_progress)
        print('\nDone.')
        flush()

    except urllib.error.HTTPError as e:
        print('Error downloading : ' + e.reason); exit(1)
