...
class EMI_CALCULATOR(object):
    ...
    def calc_interest_rate(self):
        ...
        if self.Interest_rate > 1:
            ...
            print(
            "You have not entered The interest rate correctly ,please try again ")
        pass
    ...
    def calc_emi(self):
        ...
        try:
            ...
            print(
            "You have not entered Loan amount (OR) payment period (OR) interest rate  correctly,Please enter and try again. "
            )
        try:
            self.Month_Payment = (self.Loan_amount * pow((self.Interest_rate / 12) + 1,
                                                         (self.Payment_period)) * self.Interest_rate / 12) / (
                                             pow(self.Interest_rate / 12 + 1,
                                                 (self.Payment_period)) - 1)

        except ZeroDivisionError:
            print(
            "ERROR!! ZERO DIVISION ERROR , Please enter The Interest rate correctly and Try again."
            )
        else:
            print(
            "Monthly Payment is : %r" % self.Month_Payment
            )
        pass
